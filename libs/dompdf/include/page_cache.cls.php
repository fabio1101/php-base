...
class Page_Cache {
  ...
  function init() {
    if ( is_null(self::$__connection) ) {
      ...
      if ( !self::$__connection = mysql_connect($con_str) )
  ...
  private static function __query($sql) {
    if ( !($res = mysql_query(self::$__connection, $sql)) )
      throw new Exception(mysql_last_error(self::$__connection));
    return $res;
  }
  
  static function store_page($id, $page_num, $data) {
    $where = "WHERE id='" . mysql_escape_string($id) . "' AND ".
      "page_num=". mysql_escape_string($page_num);
    ...
    if ( $row ) 
      self::__query("UPDATE page_cache SET data='" . mysql_escape_string($data) . "' " . $where);
    else 
      self::__query("INSERT INTO page_cache (id, page_num, data) VALUES ('" . mysql_escape_string($id) . "', ".
                     mysql_escape_string($page_num) . ", ".
  ...
  static function store_fonts($id, $fonts) {
    ...
    self::__query("DELETE FROM page_fonts WHERE id='" . mysql_escape_string($id) . "'");

    foreach (array_keys($fonts) as $font)
      self::__query("INSERT INTO page_fonts (id, font_name) VALUES ('" .
                    mysql_escape_string($id) . "', '" . mysql_escape_string($font) . "')");
    self::__query("COMMIT");
  }
  
//   static function retrieve_page($id, $page_num) {

//     $res = self::__query("SELECT data FROM page_cache WHERE id='" . mysql_escape_string($id) . "' AND ".
//                           "page_num=". mysql_escape_string($page_num));

//     $row = mysql_fetch_assoc($res);

//     return mysql_unescape_bytea($row["data"]);
    
//   }

  static function get_page_timestamp($id, $page_num) {
    $res = self::__query("SELECT timestamp FROM page_cache WHERE id='" . mysql_escape_string($id) . "' AND ".
                          "page_num=". mysql_escape_string($page_num));
  ...
  static function insert_cached_document(CPDF_Adapter $pdf, $id, $new_page = true) {
    $res = self::__query("SELECT font_name FROM page_fonts WHERE id='" . mysql_escape_string($id) . "'");
    ...
    $res = self::__query("SELECT data FROM page_cache WHERE id='" . mysql_escape_string($id) . "'");
